...
function show_custom_logo( $size = 'medium' ) {
    ...
    $html       = sprintf( '<a href="/' . pll_current_language() . '" class="custom-logo-link" rel="home" title="%2$s" itemscope>%3$s</a>', esc_url( home_url( '/' ) ), get_bloginfo( 'name' ), $logo_image, $link );
...
function bootstrap_scripts_and_styles() {
	if ( ! is_admin() ) {
        ...
        wp_enqueue_style( 'splide', get_template_directory_uri() . '/assets/css/splide-core.min.css', null, null );
        ...
        wp_enqueue_script('aos_js', get_template_directory_uri() . '/assets/js/plugins/aos.js', array( 'jquery' ), true );
...
//        wp_register_script( 'splide', 'https://cdn.jsdelivr.net/npm/@splidejs/splide@4.1.4/dist/js/splide.min.js', null, null, true );
//        wp_enqueue_script('splide');
//
//        wp_register_script( 'splide-autoscroll', 'https://cdn.jsdelivr.net/npm/@splidejs/splide-extension-auto-scroll@0.5.3/dist/js/splide-extension-auto-scroll.min.js', null, null, true );
...
add_image_size( 'partners_logo', 435, 0, false );
...
function yoast_seo_robots_remove_single( $robots ) {
	if ( is_category('uncategorized') || is_category('uncategorized-en') || is_search()) { 
